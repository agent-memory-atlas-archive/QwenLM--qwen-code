...
按现有规则解析的有效审批模式：default/plan 映射为只读，auto/auto-edit 映射为
workspace-write，yolo 映射为原生沙箱绕过。普通可信会话使用 auto；可信目录中的
default 父会话在没有代理覆盖设置时解析为 auto-edit。其他有效模式在启动前失败。审批请求和
...
清理失败传递到公共生命周期；初始进程树快照竞争沿用 ACP 的处理，报告无法证明
